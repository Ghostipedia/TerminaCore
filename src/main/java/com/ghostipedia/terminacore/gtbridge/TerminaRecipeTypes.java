...
public class TerminaRecipeTypes {
    ...
    public static final GTRecipeType NAQUAHINE_REACTOR = GTRecipeTypes.register("naquahine_reactor", GTRecipeTypes.MULTIBLOCK)
            .addDataInfo(data -> {
                ...
                return LocalizationUtils.format("terminacore.recipe.minField", minStrength);
            })
            .addDataInfo(data -> {
                int decayRate = data.getInt("decay_rate");
                if (!data.getBoolean("per_tick")) {
                    return LocalizationUtils.format("terminacore.recipe.fieldSlam", decayRate);
                }
                return LocalizationUtils.format("terminacore.recipe.fieldDecay", decayRate);
            })
    ...
    public static final GTRecipeType LIFE_FORCE_MANIPULATOR= GTRecipeTypes.register("life_force_manipulator", GTRecipeTypes.MULTIBLOCK)
            ...
            .setSound(GTSoundEntries.REPLICATOR) //TODO - Sounds
            .setProgressBar(GuiTextures.PROGRESS_BAR_ARROW_MULTIPLE, ProgressTexture.FillDirection.LEFT_TO_RIGHT);
    /*TODO: Neutron Forge Pressure/Heat Buildup Mechanic, feeding it astronomically large amounts of plasma allow it to unlock 'TERMINA PARALLELS' - Which Allow MULTIPLE UNIQUE RECIPES to run at once.*/
